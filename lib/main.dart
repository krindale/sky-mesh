...
class _HomePageState extends State<HomePage>
  ...
  void initState() {
    ...
    _animationController = AnimationController(
      duration: const Duration(milliseconds: 300), // 더 부드러운 전환을 위해 증가
      vsync: this,
    );
    _fadeAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: _animationController,
        curve: Curves.easeInOut, // 더 자연스러운 커브
      ),
    );
  ...
  Future<void> _loadWeatherData() async {
    try {
      ...
      bool shouldAnimate =
          currentImagePath.isNotEmpty && currentImagePath != newImagePath;
  ...
  Future<void> _loadRandomWeatherData() async {
    try {
      ...
      try {
        if (randomWeatherData.latitude != null &&
            randomWeatherData.longitude != null) {
          randomHourlyWeather = await _weatherService
              .getHourlyWeatherByCoordinates(
                randomWeatherData.latitude!,
                randomWeatherData.longitude!,
              );
          randomWeeklyWeather = await _weatherService
              .getWeeklyWeatherByCoordinates(
                randomWeatherData.latitude!,
                randomWeatherData.longitude!,
              );
        }
  ...
  Future<void> _refreshCurrentWeather() async {
    ...
    try {
      ...
      try {
        ...
        } else if (updatedWeather.latitude != null &&
            updatedWeather.longitude != null) {
          updatedHourlyWeather = await _weatherService
              .getHourlyWeatherByCoordinates(
                updatedWeather.latitude!,
                updatedWeather.longitude!,
              );
          updatedWeeklyWeather = await _weatherService
              .getWeeklyWeatherByCoordinates(
                updatedWeather.latitude!,
                updatedWeather.longitude!,
              );
              ...
              image: currentImagePath.isNotEmpty
                  ? DecorationImage(
                      ...
                      onError: (exception, stackTrace) {
                        debugPrint(
                          '❌ Failed to load current image: $currentImagePath',
                        );
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      image: DecorationImage(
                        ...
                        onError: (exception, stackTrace) {
                          debugPrint(
                            'Failed to load next image: $nextImagePath',
                          );
                        },
                      ),
                    ),
